...
void createHardcodedTestData()
{
	...
	app.GetStore().AddToGames(new Game("The Witness", "Explore a nice island and solve puzzles.", 2999, 5, 0));
	app.GetStore().AddToGames(new Game("Braid", "A time twisting puzzle game.", 499, 15, 1));
	app.GetStore().AddToGames(new Game("Factorio", "Build a complicated factory in space.", 1599, 12, 2));
	app.GetStore().AddToGames(new Game("LIMBO", "Watch out for that spider.", 299, 12, 3));
	app.GetStore().AddToGames(new Game("INSIDE", "What are those scientists even doing?!", 1299, 15, 4));
	app.GetStore().AddToGames(new Game("Portal 2", "Play around with physics. Shoot the moon.", 1999, 15, 5));
	app.GetStore().AddToGames(new Game("Half Life 3", "It's never coming out.", 5999, 18, 6));
	app.GetStore().AddToGames(new Game("NUVAVULT", "A game where 2D and 3D collide.", 299, 18, 7));
	app.GetStore().AddToGames(new Game("Brothers", "Split your brain into two thumbs.", 799, 15, 8));
	//// Create some users
